...
# ==============================================================================
# 0. CUSTOM ACTIVATIONS: PRELU (CẦN BIẾT SỐ LƯỢNG KÊNH)
# ==============================================================================
def get_activation(act_type, channels=1):
    if act_type == 'prelu':
        # Mỗi kênh sẽ tự học một hệ số góc 'a' riêng biệt
...
# ==============================================================================
# 2. ENCODER TỐI GIẢN (CẬP NHẬT TRUYỀN SỐ KÊNH VÀO ACTIVATION)
# ==============================================================================
class Straight3x3Block(nn.Module):
    def __init__(self, dim, act_type='prelu'): 
        super().__init__()
        ...
        # Truyền số kênh (dim) vào để tạo PReLU channel-wise
        self.act = get_activation(act_type, channels=dim)
...
class EncoderBlock(nn.Module):
    def __init__(self, in_c, out_c, act_type='prelu'):
        ...
        # Truyền out_c vào để tạo PReLU
        self.act = get_activation(act_type, channels=out_c)
...
class ConcatDecoderBlock_NoAttn(nn.Module):
    def __init__(self, in_c, skip_c, out_c, act_type='prelu'):
        super().__init__()
        ...
        self.refine = nn.Sequential(
            ...
            nn.BatchNorm2d(gc), 
            get_activation(act_type, channels=gc), # Kích hoạt bên trong Sequential
...
# ==============================================================================
# 4. MẠNG ABLATION 10: ALL PRELU
# ==============================================================================
class Ablation10_PReLU(nn.Module):
    def __init__(self, num_classes=1, input_size=128):
        super().__init__()
        
        self.conv_in = nn.Conv2d(3, 16, kernel_size=3, padding=1)
        
        self.e1 = EncoderBlock(16, 32,   act_type='prelu')   
        self.e2 = EncoderBlock(32, 64,   act_type='prelu')   
        self.e3 = EncoderBlock(64, 128,  act_type='prelu') 
        self.e4 = EncoderBlock(128, 128, act_type='prelu') 
        
        aux_dim = 32
        self.aux_head = nn.Sequential(
            nn.Conv2d(128, aux_dim, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(aux_dim),
            nn.PReLU(num_parameters=aux_dim), # Thay thành PReLU cho Aux
            nn.Conv2d(aux_dim, num_classes, kernel_size=1)
        )
        
        self.bottleneck = SerialBottleneck_NoAttn(128, act_type='prelu')
        
        self.d4 = ConcatDecoderBlock_NoAttn(in_c=128, skip_c=128, out_c=128, act_type='prelu') 
        self.d3 = ConcatDecoderBlock_NoAttn(in_c=128, skip_c=128, out_c=64,  act_type='prelu')  
        self.d2 = ConcatDecoderBlock_NoAttn(in_c=64,  skip_c=64,  out_c=32,  act_type='prelu')   
        self.d1 = ConcatDecoderBlock_NoAttn(in_c=32,  skip_c=32,  out_c=16,  act_type='prelu')   
...
def build_model(num_classes=1, input_size=128):
    return Ablation10_PReLU(num_classes=num_classes, input_size=input_size)