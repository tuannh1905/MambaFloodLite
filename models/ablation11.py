...
# ==============================================================================
# 0. CUSTOM ACTIVATIONS: GELU (GAUSSIAN ERROR LINEAR UNIT)
# ==============================================================================
def get_activation(act_type, channels=1):
    if act_type == 'gelu':
        # GELU không có tham số inplace trong PyTorch
        return nn.GELU()
    elif act_type == 'prelu':
...
# ==============================================================================
# 2. ENCODER TỐI GIẢN (ALL GELU)
# ==============================================================================
class Straight3x3Block(nn.Module):
    def __init__(self, dim, act_type='gelu'): 
        super().__init__()
...
class EncoderBlock(nn.Module):
    def __init__(self, in_c, out_c, act_type='gelu'):
        super().__init__()
...
# ==============================================================================
# 3. DECODER & BOTTLE-NECK (ALL GELU)
# ==============================================================================
class ConcatDecoderBlock_NoAttn(nn.Module):
    def __init__(self, in_c, skip_c, out_c, act_type='gelu'):
        super().__init__()
...
# ==============================================================================
# 4. MẠNG ABLATION 11: ALL GELU
# ==============================================================================
class Ablation11_GELU(nn.Module):
    def __init__(self, num_classes=1, input_size=128):
        super().__init__()
        
        self.conv_in = nn.Conv2d(3, 16, kernel_size=3, padding=1)
        
        # Truyền act_type='gelu' cho toàn bộ mạng
        self.e1 = EncoderBlock(16, 32,   act_type='gelu')   
        self.e2 = EncoderBlock(32, 64,   act_type='gelu')   
        self.e3 = EncoderBlock(64, 128,  act_type='gelu') 
        self.e4 = EncoderBlock(128, 128, act_type='gelu') 
        
        aux_dim = 32
        self.aux_head = nn.Sequential(
            nn.Conv2d(128, aux_dim, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(aux_dim),
            nn.GELU(), # Đổi luôn ở Aux Head thành GELU
            nn.Conv2d(aux_dim, num_classes, kernel_size=1)
        )
        
        self.bottleneck = SerialBottleneck_NoAttn(128, act_type='gelu')
        
        self.d4 = ConcatDecoderBlock_NoAttn(in_c=128, skip_c=128, out_c=128, act_type='gelu') 
        self.d3 = ConcatDecoderBlock_NoAttn(in_c=128, skip_c=128, out_c=64,  act_type='gelu')  
        self.d2 = ConcatDecoderBlock_NoAttn(in_c=64,  skip_c=64,  out_c=32,  act_type='gelu')   
        self.d1 = ConcatDecoderBlock_NoAttn(in_c=32,  skip_c=32,  out_c=16,  act_type='gelu')   
...
def build_model(num_classes=1, input_size=128):
    return Ablation11_GELU(num_classes=num_classes, input_size=input_size)