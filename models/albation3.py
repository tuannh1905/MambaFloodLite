...
# ==============================================================================
# 2. TINY-UAFM (Hòa trộn thông minh - Lần này dùng cho Encoder)
# ==============================================================================
...
# ==============================================================================
# 3. AXIAL-PFCU-DG BLOCK CẢI TIẾN (Có UAFM bên trong)
# ==============================================================================
...
class Axial_PFCU_DG(nn.Module):
    def __init__(self, dim, mixer_kernel=(5, 5)):
        ...
        # BẢN A: Cấy Tiny-UAFM vào Encoder để trộn Nhánh Bối Cảnh và Nhánh Chi Tiết
        self.uafm = TinyUAFM(in_c=dim, skip_c=dim, out_c=dim)
    ...
    def forward(self, x):
        ...
        # Thay vì cộng (+), dùng UAFM để lọc ranh giới ngay từ Encoder
        fused_all = self.uafm(x_up=fused_context, x_skip=guided_details)
        
        out = self.act(fused_all)
        return self.coord_att(out)

# ==============================================================================
# 4. ENCODER, BỘT LỌC DECODER (Phép cộng thô sơ) & BOTTLENECK
# ==============================================================================
...
class DecoderBlock(nn.Module):
    def __init__(self, in_c, out_c, mixer_kernel=(5, 5)):
        ...
        # BẢN A: Bỏ UAFM ở Decoder, dùng phép ép kênh 1x1 để chuẩn bị cho phép cộng (+)
        self.reduce_up = nn.Conv2d(in_c, out_c, kernel_size=1, bias=False) if in_c != out_c else nn.Identity()
        
        self.pw_down = nn.Conv2d(out_c, gc, kernel_size=1, bias=False)
        self.pfcu_dg = Axial_PFCU_DG(gc, mixer_kernel=mixer_kernel) # pfcu_dg này sẽ dùng UAFM bên trong nó
    ...
    def forward(self, x, skip):
        ...
        # Đảm bảo cùng Shape trước khi cộng
        if x.shape[2:] != skip.shape[2:]:
            x = F.interpolate(x, size=skip.shape[2:], mode='bilinear', align_corners=False)
            
        # BẢN A: PHÉP CỘNG (+) THÔ SƠ THAY CHO UAFM Ở SKIP CONNECTION
        x = x + skip
...
# 5. MẠNG CHÍNH
# ==============================================================================
class ULiteModel_Ablation_FusionSwap(nn.Module):
...
def build_model(num_classes=1):
    return ULiteModel_Ablation_FusionSwap(num_classes=num_classes)