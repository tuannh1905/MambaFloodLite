...
# - Loại bỏ hoàn toàn nội suy 'bilinear'. Dùng Nearest + DW Conv ở Decoder.
# - ✓ ĐÃ CẬP NHẬT: BottleNeck dùng AdaptiveAvgPool2d để hỗ trợ FCN (Kích thước ảnh động).
# - ECABlock dùng explicit reshape chống lỗi Batch Size = 1.
...
class ECABlock(nn.Module):
    ...
    Dùng AdaptiveAvgPool2d(1) tĩnh và explicit reshape, an toàn 100% cho ONNX.
    """
    ...
    def forward(self, x):
        B, C, _, _ = x.shape
        y = self.avg_pool(x).reshape(B, 1, C)              # (B, 1, C)
        y = self.sigmoid(self.conv(y))                     # (B, 1, C)
        y = y.reshape(B, C, 1, 1).expand_as(x)             # (B, C, H, W)
        return x * y
...
class NearestUpsample(nn.Module):
    """
    Nearest upsample + 1 depthwise conv 3x3 để smooth artifacts.
...
class BottleNeckBlock_v2(nn.Module):
    """
    Dùng AdaptiveAvgPool2d với output_size cố định thay AvgPool2d(kernel_size=N).
    → Hỗ trợ mọi input resolution, không hardcode input_size (Fully Convolutional).
    Shape vẫn static trong ONNX nếu trace với fixed input dummy.
    """
    def __init__(self, dim, max_dim=128):
        ...
        self.pool1 = nn.Sequential(
            nn.AdaptiveAvgPool2d((1, 1)),  # always (1,1)
            nn.Conv2d(dim, hid, 1, bias=False),
            nn.BatchNorm2d(hid), nn.ReLU6(inplace=True)
        )
        self.pool2 = nn.Sequential(
            nn.AdaptiveAvgPool2d((2, 2)),  # always (2,2)
            nn.Conv2d(dim, hid, 1, bias=False),
            nn.BatchNorm2d(hid), nn.ReLU6(inplace=True)
        )
        self.pool3 = nn.Sequential(
            nn.AdaptiveAvgPool2d((4, 4)),  # thêm scale thứ 3
            nn.Conv2d(dim, hid, 1, bias=False),
            nn.BatchNorm2d(hid), nn.ReLU6(inplace=True)
        )
    ...
    def forward(self, x):
        ...
        # ✓ ĐÃ SỬA: Thay mode='bilinear' bằng mode='nearest' để đồng bộ hóa 100% MCU-Friendly
        x1 = F.interpolate(self.pool1(x), size=(H, W), mode='nearest')
        x2 = F.interpolate(self.pool2(x), size=(H, W), mode='nearest')
        x3 = F.interpolate(self.pool3(x), size=(H, W), mode='nearest')
...
# ==============================================================================
# 5. MẠNG CHÍNH (LITEV8 - ADAPTIVE FCN)
# ==============================================================================
class ULiteModel_MCU(nn.Module):
    def __init__(self, num_classes=1):
        ...
        # ✓ ĐÃ SỬA: Trả lại cấu hình BottleNeckBlock_v2 hỗ trợ FCN
        self.b4 = BottleNeckBlock_v2(256, max_dim=128)
