...
class AddDecoderStageReLU6(nn.Module):
    # Ablation 20: hợp nhất skip-connection bằng phép cộng (add) thay vì concat.
    def __init__(self, in_channels: int, skip_channels: int, out_channels: int):
        ...
        self.skip_proj = (
            nn.Sequential(
                nn.Conv2d(skip_channels, in_channels, kernel_size=1, bias=False),
    ...
    def forward(self, x: torch.Tensor, skip: torch.Tensor) -> torch.Tensor:
        fused = self.upsample(x) + self.skip_proj(skip)
...
class FSENetSkipAdd(nn.Module):
    # Ablation 20: FSENet với decoder hợp nhất skip-connection bằng cộng (add) thay vì concat.
    ...
    def __init__(self, num_classes: int = 1, aux_hidden_channels: int = 64):
        ...
        self.aux_head = AuxiliaryBoundaryHeadReLU6(c4, aux_hidden_channels, num_classes)

        self.bottleneck = SerialBottleneckReLU6(c4)

        d4, d3, d2, d1 = self.DECODER_CHANNELS
        self.d4 = AddDecoderStageReLU6(in_channels=c4, skip_channels=c4, out_channels=d4)
        self.d3 = AddDecoderStageReLU6(in_channels=d4, skip_channels=c3, out_channels=d3)
        self.d2 = AddDecoderStageReLU6(in_channels=d3, skip_channels=c2, out_channels=d2)
        self.d1 = AddDecoderStageReLU6(in_channels=d2, skip_channels=c1, out_channels=d1)
...
def build_model(num_classes: int = 1, **kwargs) -> FSENetSkipAdd:
    return FSENetSkipAdd(num_classes=num_classes, **kwargs)
