...
class ECABlock(nn.Module):
    def __init__(self, channels, k_size=3):
        ...
        self.hardsigmoid = nn.Hardsigmoid() # ✓ THAY Sigmoid bằng Hardsigmoid

    def forward(self, x):
        B, C, _, _ = x.shape
        # ✓ Dùng REDUCE_MEAN tĩnh tuyệt đối, MCU nào cũng hỗ trợ
        y = torch.mean(x, dim=[2, 3], keepdim=True).reshape(B, 1, C)              
        y = self.hardsigmoid(self.conv(y))                     
        y = y.reshape(B, C, 1, 1) # Bỏ .expand_as(x) vì PyTorch tự động Broadcast
        return x * y
