...
# CASE F2 (PROPOSED 2): LIGHTWEIGHT DYNAMIC SCALAR FUSION (LDSF)
# - Khôi phục toàn bộ Encoder về chuẩn Baseline (chuỗi Serial Multi-scale).
# - Lược bỏ hoàn toàn Spatial Attention ở Bottleneck.
# - Tại Decoder: Sử dụng một MLP siêu nhỏ (hoạt động trên 1x1 vector sau GAP)
#   để tự động xuất ra một tỉ lệ Alpha ∈ (0, 1) cho từng bức ảnh.
# ==============================================================================
...
# ==============================================================================
# 4. DECODER (LDSF) & BOTTLE-NECK 
# ==============================================================================
class LDSF_DecoderBlock(nn.Module):
    def __init__(self, in_c, skip_c, out_c, act_type='hswish'):
        ...
        # [ĐỀ XUẤT 2: BỘ CỔNG SCALAR HỌC ĐỘNG (LDSF)]
        # Thiết kế MLP 2 lớp với Global Average Pooling.
        # Số lượng Params cực nhỏ: (skip_c*2 * mid_c) + (mid_c * 1)
        mid_c = max(4, skip_c // 2)
        self.ratio_decider = nn.Sequential(
            nn.AdaptiveAvgPool2d(1), # Ép kích thước không gian về 1x1
            nn.Conv2d(skip_c * 2, mid_c, kernel_size=1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(mid_c, 1, kernel_size=1, bias=False),
            nn.Sigmoid() # Đảm bảo đầu ra Alpha luôn nằm trong (0, 1)
        )
    ...
    def forward(self, x, skip):
        ...
        # Nối đặc trưng lại để bộ Ratio Decider đánh giá sự tương quan
        concat_feat = torch.cat([up_feat, skip], dim=1)
        
        # Tính toán Alpha động (Image-dependent). Output shape: (Batch, 1, 1, 1)
        alpha = self.ratio_decider(concat_feat)
...
# ==============================================================================
# 5. MẠNG CHÍNH PICO-UNET V4 (SMART MUSCLE - LDSF EDITION)
# ==============================================================================
class PicoUNet_v4_Edge(nn.Module):
    def __init__(self, num_classes=1, input_size=128):
        ...
        # [SỬ DỤNG KHỐI DECODER CÓ BỘ CỔNG SCALAR HỌC ĐỘNG]
        self.d4 = LDSF_DecoderBlock(in_c=192, skip_c=192, out_c=128, act_type='hswish') 
        self.d3 = LDSF_DecoderBlock(in_c=128, skip_c=192, out_c=64,  act_type='hswish')  
        self.d2 = LDSF_DecoderBlock(in_c=64,  skip_c=128, out_c=32,  act_type='hswish')   
        self.d1 = LDSF_DecoderBlock(in_c=32,  skip_c=64,  out_c=16,  act_type='hswish')   
