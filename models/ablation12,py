...
# ==============================================================================
# 0. CUSTOM ACTIVATIONS: MISH (YOLOV4'S FAVORITE)
# ==============================================================================
def get_activation(act_type, channels=1):
    if act_type == 'mish':
        # PyTorch hỗ trợ Mish native từ phiên bản 1.9+
...
# ==============================================================================
# 2. ENCODER TỐI GIẢN (ALL MISH)
# ==============================================================================
class Straight3x3Block(nn.Module):
    def __init__(self, dim, act_type='mish'): 
        super().__init__()
...
class EncoderBlock(nn.Module):
    def __init__(self, in_c, out_c, act_type='mish'):
        super().__init__()
...
# ==============================================================================
# 3. DECODER & BOTTLE-NECK (ALL MISH)
# ==============================================================================
class ConcatDecoderBlock_NoAttn(nn.Module):
    def __init__(self, in_c, skip_c, out_c, act_type='mish'):
        super().__init__()
        ...
        self.refine = nn.Sequential(
            ...
            nn.BatchNorm2d(gc), 
            get_activation(act_type, channels=gc), 
...
# ==============================================================================
# 4. MẠNG ABLATION 12: ALL MISH
# ==============================================================================
class Ablation12_Mish(nn.Module):
    def __init__(self, num_classes=1, input_size=128):
        super().__init__()
        
        self.conv_in = nn.Conv2d(3, 16, kernel_size=3, padding=1)
        
        # Truyền act_type='mish' cho toàn bộ mạng
        self.e1 = EncoderBlock(16, 32,   act_type='mish')   
        self.e2 = EncoderBlock(32, 64,   act_type='mish')   
        self.e3 = EncoderBlock(64, 128,  act_type='mish') 
        self.e4 = EncoderBlock(128, 128, act_type='mish') 
        
        aux_dim = 32
        self.aux_head = nn.Sequential(
            nn.Conv2d(128, aux_dim, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(aux_dim),
            nn.Mish(inplace=True), # Đổi luôn ở Aux Head thành Mish
            nn.Conv2d(aux_dim, num_classes, kernel_size=1)
        )
        
        self.bottleneck = SerialBottleneck_NoAttn(128, act_type='mish')
        
        self.d4 = ConcatDecoderBlock_NoAttn(in_c=128, skip_c=128, out_c=128, act_type='mish') 
        self.d3 = ConcatDecoderBlock_NoAttn(in_c=128, skip_c=128, out_c=64,  act_type='mish')  
        self.d2 = ConcatDecoderBlock_NoAttn(in_c=64,  skip_c=64,  out_c=32,  act_type='mish')   
        self.d1 = ConcatDecoderBlock_NoAttn(in_c=32,  skip_c=32,  out_c=16,  act_type='mish')   
...
def build_model(num_classes=1, input_size=128):
    return Ablation12_Mish(num_classes=num_classes, input_size=input_size)