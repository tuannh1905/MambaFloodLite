# Model K + LDA Attention + Gated Skip Connections + Strip Pooling Module (SPM)
# LDA được cắm sau stage 2 và stage 3
# Gated Skip Connections truyền thông tin từ Stage 1 và Stage 2 sang Decoder
# SPM được đặt tại bottleneck sau stage 3 để bắt ngữ cảnh dải (strip context)
...
# ─────────────────────────────────────────────
# Module cải tiến 1: LDA Attention
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# Module cải tiến 2: Gated Skip Connection
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# Module cải tiến 3: Strip Pooling Module (SPM) (NEW)
# ─────────────────────────────────────────────

class StripPooling(nn.Module):
    """Bắt ngữ cảnh dải ngang và dọc cho các cấu trúc lũ lụt kéo dài"""
    ...
    def forward(self, x):
        ...
        xh = self.pool_h(x)
        xh = self.conv_h(xh)
        xh = xh.expand(-1, -1, h, w)
        
        # Nhánh dải ngang
        xw = self.pool_w(x)
        xw = self.conv_w(xw)
        xw = xw.expand(-1, -1, h, w)
...
# ─────────────────────────────────────────────
# Model cải tiến: LDA + Gated Skip + SPM
# ─────────────────────────────────────────────
...
class ENetModel(nn.Module):
    def __init__(self, in_channels=3, num_classes=1):
        ...
        # BOTTLENECK: Strip Pooling Module (NEW)
        self.spm = StripPooling(128)

        # Decoder Stage 4
        self.attn_before_up4 = LDA(128)
        self.gated_skip_2 = GatedSkipConnection(enc_channels=128, dec_channels=128)
        ...
        self.gated_skip_1 = GatedSkipConnection(enc_channels=64, dec_channels=64)
...
if __name__ == "__main__":
    ...
    spm_p = sum(p.numel() for p in model.spm.parameters())

    print(f"Input   : {tuple(dummy.shape)}")
    print(f"Output  : {tuple(out.shape)}")
    print(f"Params (Total)     : {total:,}")
    print(f"Params (SPM Module): {spm_p:,} ({100*spm_p/total:.2f}%)")
