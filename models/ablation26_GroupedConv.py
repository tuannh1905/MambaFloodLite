...
class GroupedConv(nn.Module):
    def __init__(self, dim, kernel_size=3, groups=4):
        ...
        while dim % g != 0:
            g -= 1
        self.conv = nn.Conv2d(dim, dim, kernel_size=kernel_size, padding=kernel_size//2, groups=g, bias=False)
...
class Grouped3x3Block(nn.Module):
    def __init__(self, dim, act_type='relu6', groups=4):
        super().__init__()
        self.gc1 = GroupedConv(dim, 3, groups)
        self.gc2 = GroupedConv(dim, 3, groups)
        self.gc3 = GroupedConv(dim, 3, groups)
        self.pw_fuse = nn.Conv2d(dim, dim, 1, bias=False)
        self.bn_fuse = nn.BatchNorm2d(dim)
        self.act = get_activation(act_type)
    def forward(self, x):
        return self.act(self.bn_fuse(self.pw_fuse(self.gc3(self.gc2(self.gc1(x))))) + x)

class EncoderBlock(nn.Module):
    def __init__(self, in_c, out_c, act_type='relu6'):
        super().__init__()
        self.block = Grouped3x3Block(in_c, act_type)
...
class Ablation26_GroupedConv(nn.Module):
...
def build_model(num_classes=1, input_size=128):
    return Ablation26_GroupedConv(num_classes=num_classes, input_size=input_size)
