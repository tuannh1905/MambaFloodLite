...
class DecoderStageNoShortcut(nn.Module):
...
class FSENetDecoderNoShortcut(nn.Module):
    # Ablation: decoder refinement block without the parallel linear
    # shortcut path (main output = refine(fused) only, no residual sum).
    ...
    def __init__(self, num_classes: int = 1, aux_hidden_channels: int = 64):
        ...
        self.d4 = DecoderStageNoShortcut(in_channels=c4, skip_channels=c4, out_channels=d4)
        self.d3 = DecoderStageNoShortcut(in_channels=d4, skip_channels=c3, out_channels=d3)
        self.d2 = DecoderStageNoShortcut(in_channels=d3, skip_channels=c2, out_channels=d2)
        self.d1 = DecoderStageNoShortcut(in_channels=d2, skip_channels=c1, out_channels=d1)
...
def build_model(num_classes: int = 1, **kwargs) -> FSENetDecoderNoShortcut:
    return FSENetDecoderNoShortcut(num_classes=num_classes, **kwargs)
