...
from .propose_model.module.csam import CSAM


class UpSamplingCSAM(nn.Module):
    """UpSampling + CSAM attention trên skip connection"""
    def __init__(self, in_c, out_c):
        super().__init__()
        self.up = nn.ConvTranspose2d(in_c, in_c // 2, kernel_size=2, stride=2)
        self.csam = CSAM(in_c // 2)
        self.conv = DoubleConv(in_c, out_c)

    def forward(self, x1, x2):
        x1 = self.up(x1)
        x2 = self.csam(x2)  # attention trên skip
...
class UNetCSAM(nn.Module):
    """
    Variant 5: UNet + AxialDW + DualVSS + EESP + CSAM Decoder
    Thay UpSampling -> UpSamplingCSAM
    """
    def __init__(self, in_channels=3, num_classes=1, features=[64, 128, 256, 512]):
        ...
        # Decoder: thay UpSampling -> UpSamplingCSAM
...
def build_model(num_classes=1):
    return UNetCSAM(in_channels=3, num_classes=num_classes)