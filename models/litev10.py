...
class DetailGuidance(nn.Module):
    """Mỏ neo giữ ranh giới: Vẫn cần nhìn toàn bộ C kênh để không bỏ sót chi tiết"""
...
# ==============================================================================
# 3. SPLIT-AXIAL PFCU (TỐI ƯU HÓA THEO ĐỀ XUẤT MỚI)
# ==============================================================================
class Split_Axial_PFCU(nn.Module):
    """
    Chia C kênh thành 3 phần (25%, 25%, 50%) để xử lý song song.
    Giảm 66% tham số của khối Axial, ép mạng chuyên biệt hóa đặc trưng.
    """
    def __init__(self, dim, mixer_kernel=(5, 5), dilations=(1, 2, 5)):
        super().__init__()
        
        # Phân bổ kênh: Đảm bảo tổng bằng dim
        self.c1 = dim // 4        # 25% cho r=nhỏ
        self.c2 = dim // 4        # 25% cho r=vừa
        self.c3 = dim - self.c1 - self.c2 # 50% còn lại cho r=lớn (bối cảnh)
        
        # 3 nhánh giờ đây nhẹ đi rất nhiều vì chỉ xử lý 1 phần kênh
        self.branch_r1 = AxialDW(self.c1, mixer_kernel, dilation=dilations[0])
        self.branch_r2 = AxialDW(self.c2, mixer_kernel, dilation=dilations[1])
        self.branch_r3 = AxialDW(self.c3, mixer_kernel, dilation=dilations[2])
    ...
    def forward(self, x):
        # 1. SPLIT: Chia tensor theo chiều channel (dim=1)
        x1, x2, x3 = torch.split(x, [self.c1, self.c2, self.c3], dim=1)
        
        # 2. Xử lý song song từng cụm kênh chuyên biệt
        b1 = self.branch_r1(x1)
        b2 = self.branch_r2(x2)
        b3 = self.branch_r3(x3)
        
        # 3. CONCAT: Ghép lại thành kích thước C ban đầu
        cat_feat = torch.cat([b1, b2, b3], dim=1)
        
        # 4. FUSE: Nhào trộn thông tin giữa các cụm kênh
        fused_context = self.bn_fuse(self.pw_fuse(cat_feat))
        
        # 5. Cắm "Mỏ neo" ranh giới (DG Shortcut) và hàm kích hoạt
...
class EncoderBlock(nn.Module):
    def __init__(self, in_c, out_c, mixer_kernel=(5, 5), dilations=(1, 2, 5)):
        ...
        # Áp dụng Split-Axial siêu nhẹ
        self.pfcu_split = Split_Axial_PFCU(in_c, mixer_kernel=mixer_kernel, dilations=dilations)
    ...
    def forward(self, x):
        skip = self.bn(self.pfcu_split(x))
...
class DecoderBlock(nn.Module):
    def __init__(self, in_c, out_c, mixer_kernel=(5, 5), dilations=(1, 2, 5)):
        ...
        self.pfcu_split = Split_Axial_PFCU(gc, mixer_kernel=mixer_kernel, dilations=dilations)
    ...
    def forward(self, x, skip):
        x = self.up(x)
        x = self.uafm(x, skip)
        x = self.act(self.bn(self.pw_up(self.pfcu_split(self.pw_down(x))) + x))
        return x

# ==============================================================================
# 5. MẠNG CHÍNH (LITE V8 + SPLIT AXIAL + EXPONENTIAL DILATION)
# ==============================================================================
class ULiteModel_LiteV8_Split(nn.Module):
    def __init__(self, num_classes=1):
        ...
        # Áp dụng bộ Exponential Dilation cho khối Split siêu nhẹ
        ...
        # Decoder thu hồi Dilation ngược lại
...
def build_model(num_classes=1):
    return ULiteModel_LiteV8_Split(num_classes=num_classes)