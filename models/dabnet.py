...
class DABModule(nn.Module):
    """
    Depth-wise Asymmetric Bottleneck Module
    Sử dụng các lớp Conv 3x1 và 1x3 thay vì 3x3 để giảm tham số.
    """
...
class InputInjection(nn.Module):
    """
    Module tiêm ảnh gốc đã được thu nhỏ xuống các tầng sâu để bù đắp 
    sự thiếu hụt không gian (Spatial detail loss).
    """
...
class DABNet(nn.Module):
    ...
    def __init__(self, classes=1, block_1=3, block_2=6, input_size=256):
        ...
        # Classifier (Nhận 259 channels = 128 (out2) + 128 (out2_0) + 3 (down_3))
