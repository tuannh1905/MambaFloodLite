...
class AxialDW_Original(nn.Module):
    """Lõi AxialDW chuẩn (Có Identity Shortcut)"""
...
class Axial_PFCU_DG_NoAttn(nn.Module):
    """Lõi Encoder: PFCU + DG (Không Attention)"""
...
# ==============================================================================
# 2. ENCODER (PFCU+DG) | BOTTLENECK (SPP) | DECODER (CAT)
# ==============================================================================

class EncoderBlock_2_2(nn.Module):
...
class BottleNeckBlock_SPP_NoAttn(nn.Module):
    """
    TRƯỜNG HỢP 2.2: Bottleneck nâng cấp SPP-Axial.
    Gom bối cảnh đa quy mô (1x1, 2x2, 4x4) để nhìn toàn cục.
    KHÔNG sử dụng CoordAtt để dành đối chứng cho Kịch bản 3.
    """
    def __init__(self, dim, max_dim=128):
        super().__init__()
        hid = min(dim // 4, max_dim // 4)
        
        # 1. SPP Module: Adaptive Average Pooling đa cấp độ
        ...
        # 2. Tinh lọc lại bằng AxialDW
        self.axial_refine = AxialDW_Original(dim, mixer_kernel=(5, 5), dilation=1)
    ...
    def forward(self, x):
        ...
        # Ghép đặc trưng gốc với đặc trưng toàn cục
...
# ==============================================================================
# 3. MẠNG CHÍNH (ABLATION 2.2: ENCODER PFCU_DG + BOTTLENECK SPP)
# ==============================================================================

class ULiteModel_Ablation_2_2(nn.Module):
    def __init__(self, num_classes=1):
        ...
        self.e1 = EncoderBlock_2_2(16, 32, mixer_kernel=mk)
        self.e2 = EncoderBlock_2_2(32, 64, mixer_kernel=mk)
        self.e3 = EncoderBlock_2_2(64, 128, mixer_kernel=mk)
        self.e4 = EncoderBlock_2_2(128, 256, mixer_kernel=mk)
        self.e5 = EncoderBlock_2_2(256, 512, mixer_kernel=mk)

        # Bottleneck cải tiến CÓ SPP
        self.b5 = BottleNeckBlock_SPP_NoAttn(512, max_dim=128)

        # Decoder vẫn giữ thô sơ để cô lập biến SPP
...
def build_model(num_classes=1):
    return ULiteModel_Ablation_2_2(num_classes=num_classes)