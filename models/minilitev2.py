...
# - BOTTLE-NECK TĨNH: Tính trước kernel cho AvgPool2d trong __init__. 
# - ✓ ĐÃ CẬP NHẬT: Thay chập chéo (Axial/Dilation) bằng chập vuông đặc (SquareDW).
#   Sử dụng Multi-scale Depthwise (3x3, 5x5, 7x7) để NPU chạy 100% tốc độ phần cứng.
# ==============================================================================
...
class SquareDW(nn.Module):
    """
    Depthwise Convolution vuông vắn chuẩn mực (3x3, 5x5, 7x7).
    Tuyệt đối không dùng Dilation hay Asymmetric để tránh tràn Line Buffer NPU.
    """
...
class MultiScale_PFCU_DG(nn.Module):
    def __init__(self, dim):
        super().__init__()
        # ✓ Thay máu: Multi-Scale Square Depthwise (3x3, 5x5, 7x7)
...
class BottleNeckBlock_Static(nn.Module):
    ...
    def __init__(self, dim, max_dim=128, input_size=256):
        ...
        if input_size == 128:
            ...
            raise ValueError(f"input_size={input_size} chưa được hỗ trợ. Dùng 128 hoặc 256.")
        ...
        # ✓ Thay bằng SquareDW 5x5 chuẩn mực
    ...
    def forward(self, x):
        ...
        out = self.bn_refine(self.square_refine(spp))
        return self.se(out) + x

# ==============================================================================
# 5. MẠNG CHÍNH (FINAL MCU-NATIVE ARCHITECTURE)
# ==============================================================================
