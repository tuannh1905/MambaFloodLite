...
class Axial_GE_DG(nn.Module):
    def __init__(self, dim, mixer_kernel=(5, 5), dilations=(1, 2, 5), exp_ratio=1):
        ...
        # 1. GATHER: LIỀU THUỐC 1 (DS-Gather: DW 3x3 -> PW 1x1)
        self.gather = nn.Sequential(
            # Bước 1: Gom cục bộ bằng DW 3x3 để lọc nhiễu (Rất rẻ)
            nn.Conv2d(dim, dim, kernel_size=3, padding=1, groups=dim, bias=False),
            nn.BatchNorm2d(dim),
            nn.PReLU(dim),
            # Bước 2: Trộn và mở rộng kênh bằng 1x1
...
# ==============================================================================
# 5. MẠNG CHÍNH (DS-GATHER + CAPPED 192 + WIDE DILATION TẦNG ĐÁY)
# ==============================================================================
class ULiteModel_Recovery(nn.Module):
    def __init__(self, num_classes=1):
        ...
        # LIỀU THUỐC 2: Tăng Dilation ở e4 lên (1, 12, 24) để bù đắp trường thụ cảm
