...
# Giữ lại SpatialAttention nếu sau này bạn muốn dùng ở Decoder, 
# hiện tại model Proposed không dùng khối này ở Bottleneck nữa.
...
# ==============================================================================
# 3. KHỐI ENCODER "SMART MUSCLE" (BẢN GỐC TỐI ƯU SERIAL 3x3)
# ==============================================================================
...
# ==============================================================================
# 4. BOTTLENECK ĐỀ XUẤT (ORTHOGONAL ADDITIVE) & DECODER
# ==============================================================================
class OrthogonalContextBottleneck(nn.Module):
    def __init__(self, dim, act_type='hswish'):
        super().__init__()
        
        # 1. Nhánh Local: Trích xuất không gian cục bộ cơ bản (3x3)
        self.local_dw = SquareDW(dim, kernel_size=3)
        
        # 2. Nhánh Ngữ cảnh Trục Dọc (Height)
        self.proj_h = nn.Sequential(
            nn.Conv2d(dim, dim, kernel_size=1, bias=False),
            nn.BatchNorm2d(dim)
        )
        
        # 3. Nhánh Ngữ cảnh Trục Ngang (Width)
        self.proj_w = nn.Sequential(
            nn.Conv2d(dim, dim, kernel_size=1, bias=False),
            nn.BatchNorm2d(dim)
        )
        
        # 4. Trộn đặc trưng (Fusion)
    ...
    def forward(self, x):
        # Đặc trưng cục bộ: (B, C, H, W)
        feat_local = self.local_dw(x)
        
        # Ép dọc (Pool theo Width - dim 3): (B, C, H, 1)
        pool_h = torch.mean(x, dim=3, keepdim=True)
        feat_h = self.proj_h(pool_h)
        
        # Ép ngang (Pool theo Height - dim 2): (B, C, 1, W)
        pool_w = torch.mean(x, dim=2, keepdim=True)
        feat_w = self.proj_w(pool_w)
        
        # ORTHOGONAL ADDITIVE FUSION (Cộng Broadcast)
        fused = feat_local + feat_h + feat_w
...
# ==============================================================================
# 5. MẠNG CHÍNH: PICO-UNET V4 (PROPOSED ARCHITECTURE)
# ==============================================================================
class PicoUNet_v4_Proposed(nn.Module):
    def __init__(self, num_classes=1, input_size=128):
        ...
        # Encoder: Smart Muscle (Dual/Multi-Scale Serial 3x3)
        ...
        # Bottleneck: KHỐI ORTHOGONAL ADDITIVE ĐỀ XUẤT MỚI
        self.bottleneck = OrthogonalContextBottleneck(192, act_type='hswish')
        
        # Decoder: Additive Fusion
...
# ==============================================================================
# 6. HÀM KHỞI TẠO CHUẨN CHO BENCHMARK SCRIPT
# ==============================================================================
