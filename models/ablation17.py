...
class SquareDW(nn.Module):
    ...
    def forward(self, x): return self.bn(self.dw(x))
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x):
        feat = self.block(x)
        if self.same_channels: return self.act(self.down_pool(feat)), feat
        else:
...
class ConcatDecoderBlock_NoAttn(nn.Module):
    def __init__(self, in_c, skip_c, out_c, act_type='relu6'):
        ...
        gc = max(out_c // 4, 4)
        self.refine = nn.Sequential(nn.Conv2d(in_c + skip_c, gc, 1, bias=False), nn.BatchNorm2d(gc), get_activation(act_type), SquareDW(gc, 5), nn.Conv2d(gc, out_c, 1, bias=False), nn.BatchNorm2d(out_c))
...
class SerialBottleneck_NoAttn(nn.Module):
    def __init__(self, dim, act_type='relu6'):
        ...
        self.dw2 = SquareDW(dim)
        self.dw3 = SquareDW(dim)
    def forward(self, x): return x + self.dw1(x) + self.dw2(self.dw1(x)) + self.dw3(self.dw2(self.dw1(x)))

# ==============================================================================
# 4. ABLATION 17: BISENET-STYLE TASK-DECOUPLED DEEP SUPERVISION
# ==============================================================================
class Ablation17_BiSeNetStyle(nn.Module):
    def __init__(self, num_classes=1, input_size=128):
        ...
        # Aux 1: Đặt ở E2 (Tầng nông, độ phân giải cao). Nhiệm vụ: Bắt viền (Boundary)
        ...
        # Aux 2: Đặt ở E4 (Tầng sâu, ngữ nghĩa tốt). Nhiệm vụ: Gom khối nước (Semantic)
        self.aux_head_semantic = nn.Sequential(
    ...
    def forward(self, x):
        ...
        x, s3 = self.e3(x)
        x, s4 = self.e4(x) # Lấy s4 cho Semantic Aux
        
        out_semantic = None
        out_detail = None
        
        if self.training:
            # Tính toán 2 nhánh Aux
            out_detail = self.aux_head_detail(s2)
            out_semantic = self.aux_head_semantic(s4)
            
            # Phóng to về kích thước gốc
            out_detail = F.interpolate(out_detail, size=input_shape, mode='bilinear', align_corners=False)
            out_semantic = F.interpolate(out_semantic, size=input_shape, mode='bilinear', align_corners=False)
        ...
        if self.training:
            # Tuple trả về: (Nhánh chính, Nhánh ngữ nghĩa E4, Nhánh chi tiết viền E2)
            return main_out, out_semantic, out_detail
        return main_out

def build_model(num_classes=1, input_size=128):
    return Ablation17_BiSeNetStyle(num_classes=num_classes, input_size=input_size)