...
# ==============================================================================
# 4. ENCODER, DECODER & BOTTLENECK (Thuần Tích chập - No SPP)
# ==============================================================================
...
class BottleNeckBlock(nn.Module):
    """Bottleneck thuần tích chập, giữ Dilation 1, 2, 5 để chống Gridding Effect"""
    def __init__(self, dim, max_dim=128):
        ...
        self.dw3 = AxialDW(gc, mixer_kernel=(3, 3), dilation=5)
    ...
    def forward(self, x):
        x1 = self.pw1(x)
        # Nối đặc trưng từ 3 mức giãn nở khác nhau
        out = torch.cat([x1, self.dw1(x1), self.dw2(x1), self.dw3(x1)], dim=1)
        out = self.act(self.pw2(self.bn(out)))
        # Chốt tọa độ không gian trước khi giải mã
        return self.coord_att(out)
...
# ==============================================================================
# 5. MẠNG CHÍNH (ULITE-PFCU-UAFM - NO SPP)
# ==============================================================================
