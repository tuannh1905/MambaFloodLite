...
# ==============================================================================
# 3. AXIAL-PFCU-DG (HỖ TRỢ DILATION ĐỘNG)
# ==============================================================================
class Axial_PFCU_DG(nn.Module):
    """Giờ đây nhận vào tham số `dilations` dạng list để thay đổi theo Stage"""
...
class DecoderBlock(nn.Module):
    def __init__(self, in_c, out_c, mixer_kernel=(5, 5), dilations=[1, 2, 3]):
        super().__init__()
        gc = max(out_c // 4, 4)
        ...
        # Truyền dilation vào PFCU của Decoder (Dùng mức giãn nhẹ hơn Encoder)
...
# ==============================================================================
# 5. MẠNG CHÍNH (PROGRESSIVE DILATION)
# ==============================================================================
class ULiteModel_Progressive(nn.Module):
    def __init__(self, num_classes=1):
        ...
        # ÁP DỤNG PROGRESSIVE EXPANSION CHO ENCODER
        self.e1 = EncoderBlock(16,  32,  mixer_kernel=mk, dilations=[1, 2, 3])
        self.e2 = EncoderBlock(32,  64,  mixer_kernel=mk, dilations=[1, 3, 5])
        self.e3 = EncoderBlock(64,  128, mixer_kernel=mk, dilations=[1, 4, 9])
        self.e4 = EncoderBlock(128, 256, mixer_kernel=mk, dilations=[1, 8, 18])

        self.b4 = BottleNeckBlock(256, max_dim=128)

        # DECODER KHÔNG CẦN GIÃN NỞ QUÁ TO (Thu hẹp dần để khôi phục chi tiết)
        self.d4 = DecoderBlock(256, 128, mixer_kernel=mk, dilations=[1, 4, 9])
        self.d3 = DecoderBlock(128, 64,  mixer_kernel=mk, dilations=[1, 3, 5])
...
def build_model(num_classes=1):
    return ULiteModel_Progressive(num_classes=num_classes)