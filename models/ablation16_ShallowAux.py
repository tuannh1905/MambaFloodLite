...
# ==============================================================================
# 2. ENCODER TỐI GIẢN (STRAIGHT 3x3)
# ==============================================================================
...
# ==============================================================================
# 4. ABLATION 18: SHALLOW AUX (AUX Ở E2 ĐỂ HỌC VIỀN CỤC BỘ)
# ==============================================================================
class Ablation_ShallowAux(nn.Module):
    def __init__(self, num_classes=1, input_size=128):
        ...
        # ----------------------------------------------------------------------
        # SỰ KHÁC BIỆT NẰM Ở ĐÂY: NHÁNH AUX CHĨA RA TỪ TẦNG NÔNG (E2)
        # s2 có 128 channels -> Đầu vào Aux Head là 128
        # ----------------------------------------------------------------------
        aux_dim = 64
    ...
    def forward(self, x):
        ...
        x, s1 = self.e1(x)
        x, s2 = self.e2(x) # Lấy s2 (Tầng nông) để đưa vào Aux Head
        x, s3 = self.e3(x)
        x, s4 = self.e4(x)
        
        # Xử lý Aux Head (Chỉ hoạt động lúc huấn luyện)
        aux_out = None
        if self.training:
            # Truyền s2 vào nhánh phụ để ép học viền
            aux_out = self.aux_head_shallow(s2)
            # DYNAMIC INTERPOLATION: Phóng to về kích thước ảnh gốc
...
# Hàm build chuẩn cho file __init__.py / get_model của bác
