...
class FSENetShallowAux(nn.Module):
    # Ablation 18: auxiliary boundary head moved from the deepest skip
    # feature (s4) to a shallower one (s2).
    ...
    def __init__(self, num_classes: int = 1, aux_hidden_channels: int = 64):
        ...
        self.aux_head = AuxiliaryBoundaryHead(c2, aux_hidden_channels, num_classes)
    ...
    def forward(self, x: torch.Tensor):
        ...
        aux_out = self.aux_head(s2, input_size) if self.training else None
...
def build_model(num_classes: int = 1, **kwargs) -> FSENetShallowAux:
    return FSENetShallowAux(num_classes=num_classes, **kwargs)
