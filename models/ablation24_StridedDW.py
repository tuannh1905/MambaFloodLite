...
class SquareDW(nn.Module):
    def __init__(self, dim, kernel_size=3, stride=1):
        super().__init__()
        self.dw = nn.Conv2d(dim, dim, kernel_size=kernel_size, stride=stride, padding=kernel_size//2, groups=dim, bias=False)
...
class EncoderBlock_StridedDW(nn.Module):
    def __init__(self, in_c, out_c, act_type='relu6'):
        super().__init__()
        self.block = Straight3x3Block(in_c, act_type)
        self.down_dw = SquareDW(in_c, kernel_size=3, stride=2)
        self.same_channels = (in_c == out_c)
        if not self.same_channels:
            self.pw = nn.Sequential(nn.Conv2d(in_c, out_c - in_c, 1, bias=False), nn.BatchNorm2d(out_c - in_c))
            self.down_pw_dw = SquareDW(out_c - in_c, kernel_size=3, stride=2)
        self.act = get_activation(act_type)
    def forward(self, x):
        feat = self.block(x)
        if self.same_channels:
            return self.act(self.down_dw(feat)), feat
        else:
            feat_pw = self.pw(feat)
            skip = torch.cat([feat, feat_pw], dim=1)
            out = self.act(torch.cat([self.down_dw(feat), self.down_pw_dw(feat_pw)], dim=1))
            return out, skip
...
class Ablation24_StridedDW(nn.Module):
    def __init__(self, num_classes=1, input_size=128):
        super().__init__()
        self.conv_in = nn.Conv2d(3, 32, kernel_size=3, padding=1)
        self.e1 = EncoderBlock_StridedDW(32, 64)
        self.e2 = EncoderBlock_StridedDW(64, 128)
        self.e3 = EncoderBlock_StridedDW(128, 128)
        self.e4 = EncoderBlock_StridedDW(128, 128)
        aux_dim = 64
        ...
        self.d2 = ConcatDecoderBlock_NoAttn(64, 128, 32)
        self.d1 = ConcatDecoderBlock_NoAttn(32, 64, 16)
...
def build_model(num_classes=1, input_size=128):
    return Ablation24_StridedDW(num_classes=num_classes, input_size=input_size)
