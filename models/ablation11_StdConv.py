...
from models.fsenet import AuxiliaryBoundaryHead


class StandardConvBN(nn.Module):
...
class NearestUpsampleRefineStd(nn.Module):
    def __init__(self, channels: int):
        super().__init__()
        self.upsample = nn.Upsample(scale_factor=2, mode="nearest")
        self.refine = StandardConvBN(channels, kernel_size=3)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return self.refine(self.upsample(x))


class SerialStandardConvBlock(nn.Module):
    def __init__(self, channels: int):
        super().__init__()
        self.conv1 = StandardConvBN(channels, kernel_size=3)
        self.conv2 = StandardConvBN(channels, kernel_size=3)
        self.conv3 = StandardConvBN(channels, kernel_size=3)
...
class EncoderStageStd(nn.Module):
    def __init__(self, in_channels: int, out_channels: int):
        super().__init__()
        self.block = SerialStandardConvBlock(in_channels)
...
class SerialBottleneckStd(nn.Module):
    def __init__(self, channels: int):
        super().__init__()
        self.conv1 = StandardConvBN(channels, kernel_size=3)
        self.conv2 = StandardConvBN(channels, kernel_size=3)
...
class DecoderStageStd(nn.Module):
    def __init__(self, in_channels: int, skip_channels: int, out_channels: int):
        super().__init__()
        self.upsample = NearestUpsampleRefineStd(in_channels)
        ...
        self.refine = nn.Sequential(
            ...
            StandardConvBN(hidden_channels, kernel_size=5),
...
class FSENetStdConv(nn.Module):
    # Ablation 14: standard (non-depthwise) convolutions throughout, at the
    # same macro-architecture as FSENet.
    ...
    def __init__(self, num_classes: int = 1, aux_hidden_channels: int = 64):
        ...
        self.e1 = EncoderStageStd(c_stem, c1)
        self.e2 = EncoderStageStd(c1, c2)
        self.e3 = EncoderStageStd(c2, c3)
        self.e4 = EncoderStageStd(c3, c4)

        self.aux_head = AuxiliaryBoundaryHead(c4, aux_hidden_channels, num_classes)

        self.bottleneck = SerialBottleneckStd(c4)

        d4, d3, d2, d1 = self.DECODER_CHANNELS
        self.d4 = DecoderStageStd(in_channels=c4, skip_channels=c4, out_channels=d4)
        self.d3 = DecoderStageStd(in_channels=d4, skip_channels=c3, out_channels=d3)
        self.d2 = DecoderStageStd(in_channels=d3, skip_channels=c2, out_channels=d2)
        self.d1 = DecoderStageStd(in_channels=d2, skip_channels=c1, out_channels=d1)
...
def build_model(num_classes: int = 1, **kwargs) -> FSENetStdConv:
    return FSENetStdConv(num_classes=num_classes, **kwargs)
