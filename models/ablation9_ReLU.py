...
class NearestUpsampleRefineReLU(nn.Module):
...
class SerialDepthwiseBlockReLU(nn.Module):
    def __init__(self, channels: int):
        super().__init__()
...
class EncoderStageReLU(nn.Module):
    def __init__(self, in_channels: int, out_channels: int):
        super().__init__()
        self.block = SerialDepthwiseBlockReLU(in_channels)
...
class SerialBottleneckReLU(nn.Module):
    def __init__(self, channels: int):
        super().__init__()
        self.dw1 = DepthwiseConvBN(channels, kernel_size=3)
        self.dw2 = DepthwiseConvBN(channels, kernel_size=3)
...
class DecoderStageReLU(nn.Module):
    def __init__(self, in_channels: int, skip_channels: int, out_channels: int):
        super().__init__()
        self.upsample = NearestUpsampleRefineReLU(in_channels)
...
class AuxiliaryBoundaryHeadReLU(nn.Module):
...
class FSENetReLU(nn.Module):
    # Ablation 9: FSENet with standard ReLU activations everywhere instead of ReLU6.
    ...
    def __init__(self, num_classes: int = 1, aux_hidden_channels: int = 64):
        ...
        self.e1 = EncoderStageReLU(c_stem, c1)
        self.e2 = EncoderStageReLU(c1, c2)
        self.e3 = EncoderStageReLU(c2, c3)
        self.e4 = EncoderStageReLU(c3, c4)

        self.aux_head = AuxiliaryBoundaryHeadReLU(c4, aux_hidden_channels, num_classes)

        self.bottleneck = SerialBottleneckReLU(c4)

        d4, d3, d2, d1 = self.DECODER_CHANNELS
        self.d4 = DecoderStageReLU(in_channels=c4, skip_channels=c4, out_channels=d4)
        self.d3 = DecoderStageReLU(in_channels=d4, skip_channels=c3, out_channels=d3)
        self.d2 = DecoderStageReLU(in_channels=d3, skip_channels=c2, out_channels=d2)
        self.d1 = DecoderStageReLU(in_channels=d2, skip_channels=c1, out_channels=d1)
...
def build_model(num_classes: int = 1, **kwargs) -> FSENetReLU:
    return FSENetReLU(num_classes=num_classes, **kwargs)
