...
# ==============================================================================
# 1. CHANNEL ATTENTION (ECA) & TINY-UAFM
# ==============================================================================

class ECAModule(nn.Module):
    """
    TRƯỜNG HỢP 3.2: Sử dụng ECA thay cho CoordAtt ở toàn bộ các khối.
    Chỉ tập trung vào Channel, bỏ qua Spatial/Coordinate.
    """
...
class TinyUAFM(nn.Module):
    """Dung hợp thông minh cho Decoder"""
...
# ==============================================================================
# 2. AXIAL-PFCU-DG (VỚI ECA THAY CHO COORD-ATT)
# ==============================================================================
...
class Axial_PFCU_DG_ECA(nn.Module):
    """Lõi đặc trưng: Sử dụng ECA làm chốt chặn cuối cùng"""
...
# ==============================================================================
# 3. ENCODER, BOTTLENECK (SPP) & DECODER
# ==============================================================================

class EncoderBlock_3_2(nn.Module):
    def __init__(self, in_c, out_c, mixer_kernel=(5, 5)):
        super().__init__()
        self.same_channels = (in_c == out_c)
        conv_out = out_c - in_c if not self.same_channels else out_c

        self.dw = Axial_PFCU_DG_ECA(in_c, mixer_kernel=mixer_kernel)
...
class BottleNeckBlock_SPP_ECA(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.eca(self.act_out(out + spp_fused))


class DecoderBlock_3_2(nn.Module):
    def __init__(self, in_c, out_c, mixer_kernel=(5, 5)):
        super().__init__()
        gc = max(out_c // 4, 4)
        ...
        self.pfcu_dg = Axial_PFCU_DG_ECA(gc, mixer_kernel=mixer_kernel)
...
# ==============================================================================
# 4. MẠNG CHÍNH (ABLATION 3.2: WITH ECA)
# ==============================================================================

class ULiteModel_Ablation_3_2(nn.Module):
    def __init__(self, num_classes=1):
        ...
        self.e1 = EncoderBlock_3_2(16, 32, mixer_kernel=mk)
        self.e2 = EncoderBlock_3_2(32, 64, mixer_kernel=mk)
        self.e3 = EncoderBlock_3_2(64, 128, mixer_kernel=mk)
        self.e4 = EncoderBlock_3_2(128, 256, mixer_kernel=mk)
        self.e5 = EncoderBlock_3_2(256, 512, mixer_kernel=mk)

        self.b5 = BottleNeckBlock_SPP_ECA(512, max_dim=128)

        self.d5 = DecoderBlock_3_2(512, 256, mixer_kernel=mk)
        self.d4 = DecoderBlock_3_2(256, 128, mixer_kernel=mk)
        self.d3 = DecoderBlock_3_2(128, 64, mixer_kernel=mk)
        self.d2 = DecoderBlock_3_2(64, 32, mixer_kernel=mk)
        self.d1 = DecoderBlock_3_2(32, 16, mixer_kernel=mk)
...
def build_model(num_classes=1):
    return ULiteModel_Ablation_3_2(num_classes=num_classes)